    ...
    <main className="container fade-up pb-8">
      <section className="card overflow-hidden">
        ...
        <div className="p-8">
          ...
          <div className="mt-5 grid gap-2 text-sm text-[var(--brand-deep)] md:grid-cols-3">
            <p><strong>Specialty:</strong> {project.sector}</p>
            <p><strong>Clinic:</strong> {project.location}</p>
            <p><strong>Joined:</strong> {new Date(project.createdAt).toLocaleDateString()}</p>
          </div>
        </div>
      </section>
      ...
      <section className="mt-6 grid gap-6 md:grid-cols-[1.4fr_1fr]">
        <article className="card p-6">
          <h2 className="text-2xl font-bold">Doctor Highlights</h2>
        ...
        <article className="card p-6">
          ...
          <form action={`/api/public/doctors/${project.slug}/appointments`} method="post" className="mt-4 grid gap-3 text-sm">
            ...
            <input name="company" placeholder="Insurance provider (optional)" className="rounded-lg border border-[var(--line)] px-3 py-2" />
            <input name="budget" placeholder="Preferred date/time" className="rounded-lg border border-[var(--line)] px-3 py-2" />
            <textarea name="message" placeholder="Symptoms or notes" rows={4} className="rounded-lg border border-[var(--line)] px-3 py-2" />
            <button className="button button-primary">Request Appointment</button>
          </form>
        </article>
      </section>
    </main>
