            ...
            <article key={item.id} className="rounded-xl border border-[var(--line)] bg-[#fbfdff] p-4 text-sm">
              ...
              {item.date && item.time && (
                <p className="mt-1 font-semibold text-[var(--brand-deep)]">
                  Preferred: {new Date(item.date).toLocaleDateString()} at {item.time}
                </p>
              )}
              <p className="mt-1 text-[var(--muted)]">Location/Insurance: {item.company || "N/A"} • Service: {item.budget || "N/A"}</p>
